...
<div class="rbb-cart__totals rounded-2xl cart_totals border overflow-hidden <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<h2 class="rbb-cart__totals-header text-lg uppercase px-[30px] py-4"><?php esc_html_e('Cart totals', 'gearnix'); ?></h2>
	...
	<div class="rbb-cart__totals-content rounded-2xl p-[30px] shop_table shop_table_responsive">
		...
		<div class="rbb-cart__totals-sub flex items-center justify-between border-b pb-6 mb-6 cart-subtotal">
			<div class="rbb-cart__header font-bold"><?php esc_html_e('Subtotal', 'gearnix'); ?></div>
			<div class="total__price font-medium" data-title="<?php esc_attr_e('Subtotal', 'gearnix'); ?>"><?php wc_cart_totals_subtotal_html(); ?></div>
		</div>
		...
		<?php elseif ( 'yes' === get_option('woocommerce_enable_shipping_calc') && WC()->cart->needs_shipping() ) : ?>
			...
			<div class="shipping">
				<div><?php esc_html_e('Shipping', 'gearnix'); ?></div>
				<div data-title="<?php esc_attr_e('Shipping', 'gearnix'); ?>"><?php woocommerce_shipping_calculator(); ?></div>
			</div>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
				...
				$estimated_text = sprintf(' <small>' . esc_html__('(estimated for %s)', 'gearnix') . '</small>', WC()->countries->estimated_for_prefix($taxable_address[0]) . WC()->countries->countries[ $taxable_address[0] ]);
			}
		...
		<div class="order-total flex items-center justify-between mb-10">
			...
			<div class="font-extrabold text-xl" data-title="<?php esc_attr_e('Total', 'gearnix'); ?>"><?php wc_cart_totals_order_total_html(); ?></div>
		</div>
